...
export function createProtocolHandler(tools, handlers) {
  const methodMap = new Map([
    ...
    ['prompts/list', m => {
      const list = (typeof handlers._prompts === 'function') ? handlers._prompts() : (handlers._prompts || []);
      return { id: m.id, result: { prompts: list } };
    }],
    ['prompts/get', m => {
      const fn = handlers._getPrompt;
      if (typeof fn !== 'function') return { id: m.id, result: { messages: [] } };
      try {
        const r = fn(m.params);
        const wrap = val => ({ id: m.id, result: { messages: Array.isArray(val) ? val : [val] } });
        if (r && typeof r.then === 'function') return r.then(wrap).catch(e => ({ id: m.id, result: { content: [{ type: 'text', text: e?.message || String(e) }], isError: true } }));
        return wrap(r);
      } catch (e) {
        return { id: m.id, result: { content: [{ type: 'text', text: e?.message || String(e) }], isError: true } };
  ...
  return {
    handleMessage(m) {
      // Notifications (no id) are silently ignored as per JSON-RPC spec
      if (m.id == null) return null;
      const fn = methodMap.get(m.method);
      // Handle notifications that arrive with id (some clients send them)
      if (!fn && m.method && m.method.startsWith('notifications/')) return null;
      return fn ? fn(m) : null;
    }
  };
}
...
export function serve(tools, handlers) {
  ...
  function send(id, result) {
    ...
    process.stdout.write(`Content-Length: ${len}\r\nContent-Type: application/json\r\n\r\n${body}`, () => {
      // Write callback — ensures data flushed before process may exit
    });
  }
}
...
export function serveHttp(tools, handlers, port = 3100) {
  ...
  const server = http.createServer((req, res) => {
    ...
    // GET /health, GET /tools, or GET / for simple checks
    if (req.method === 'GET') {
